...
class Cuotafija {
    ...
    table() {
        ...
        for (let i = 0; i < this.plazo; i++) {
            let pago = 1 + i;
            let cut = Number((this.pagofijo(int, this.prestamo, this.plazo)).toFixed(2));
            this.No.push({ pago });
        ...
        console.log(this.cuota, 'kjjkkj');
        this.Interes[0] = { interes: Number((int * this.prestamo).toFixed(2)) };
        ...
        for (let i = 1; i < this.plazo; i++) {
            this.Interes[i] = { interes: Number((this.saldo[i - 1].saldo * int).toFixed(2)) };
